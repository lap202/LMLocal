...
export function createStreamingScheduler(streamBuffer, options = {}) {
    ...
    const processNext = async () => {
        ...
        const textToRender = streamBuffer.visibleText;
        const nextPromise = pendingPromise.then(() => callRenderer(textToRender));
        pendingPromise = nextPromise;
        await nextPromise;

        if (getRemainingWords() > 0 && isRunning) {
            scheduleNext();
        } else {
            if (timer) {
                clearTimeout(timer);
    ...
    const flush = () => {
        ...
        if (currentTaskFn) {
            pendingPromise = pendingPromise.then(() => currentTaskFn(finalText));
