...
import { createHighlightWorkerClient } from './highlight.worker.client.js';
import { createMarkDownParser, ParserType } from './markdown.parser.js';
...
import { createStreamingRenderer, StreamingMode } from './streaming.renderer.js';
import { createStreamingScheduler } from './streaming.scheduler.js';
/**
        ...
        switch (state.status) {
            ...
            case AppStatus.FINISHING: {
                this.currentAi?.finishStreaming().then(async () => {
                    this.onHighlightCode.emit(true);
                }).finally(() => {
