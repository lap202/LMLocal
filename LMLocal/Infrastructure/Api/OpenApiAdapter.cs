...
namespace LMLocal.Infrastructure.Api
{
    ...
    {
        Task<string> ListModelsRawAsync(string endpoint, CancellationToken cancellationToken);
    ...
    internal class OpenApiAdapter : IOpenApiAdapter
    {
        ...
        /// <summary>
        /// Retrieves raw JSON response for models list from the backend.
        /// Does not perform any transformation or conversion.
        /// </summary>
        public async Task<string> ListModelsRawAsync(string endpoint, CancellationToken cancellationToken)
        {
            ...
            try
            {
                using (var request = new HttpRequestMessage(HttpMethod.Get, baseUrl + endpoint))
                {
                    ...
                    using (var response = await _httpClientWrapper.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
                    {
                        ...
                        {
                            InternalLogger.Warn($"ListModelsRawAsync failed with status {response.StatusCode}");
                            return string.Empty;
                        }
